		...
		for whCat, whTbl in pairs(tempWarehouses) do -- cycle in warehouse table
			if whCat == "airports" then
				...
				for whId, whData in pairs(whTbl) do -- cycle in warehouse table
					...
					-- small interposition to update airbaseTbl
					if tblAirbases then
						for aId, aData in pairs(tblContent_a) do
							if tonumber(aData.id) == tonumber(whId) then
								for bId, bData in pairs(tblAirbases) do
									if aData.name == bData.name then
										--HOOK.writeDebugDetail(ModuleName .. ": updating tblAirbases: " .. tostring(aData.name))
										bData.unlimitedFuel = whData.unlimitedFuel
										bData.unlimitedAcf = whData.unlimitedAircrafts
										bData.unlimitedWpn= whData.unlimitedMunitions
									end
								end
							end
						end
										...
										if awId == "weapon" then
											for wpnId, wpnData in pairs(awData) do
												if type(wpnData) == 'table' then
													...
													HOOK.writeDebugBase(ModuleName .. ": updateWarehouses wpnId " .. tostring(wpnId) .. " ERROR: something has been added in the wrong way or with DCS bug")
												end
											end
										end
									end
