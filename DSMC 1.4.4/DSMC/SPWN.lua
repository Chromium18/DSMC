	...
	if missionEnv.maxDictId then
		...
		if tblSpawned then
			...
			for _id, sgData in pairs(tblSpawned) do
				...
				HOOK.writeDebugDetail(ModuleName .. ": doSpawned, checking group " .. tostring(_id))
				if string.find(_id, HOOK.EMBD_var) then
					HOOK.writeDebugDetail(ModuleName .. ": doSpawned, group " .. tostring(_id) .. " is excluded")
					oneAlive = false
				end
