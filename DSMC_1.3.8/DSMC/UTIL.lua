...
function createImagesTbl()
	...
	UTIL.dumpTable("imagesCode.lua", t)
	return t
end
...
function CTLD_integrationWeaponDB()
	...
	--UTIL.dumpTable("resource_by_unique_name.lua", rbun, "int")
	--[[
...
function createWhWeaponsDb()
	...
	if proceed == true then
		...
		for dbId, dbData in pairs(ME_DB) do
			...
			elseif dbId == "category_by_weapon_CLSID" then
				wbc = dbData
			elseif dbId == "helicopter_by_type" then
				--dumpTable("helicopter_by_typeX.lua", dbData)
				--dumpTable("helicopter_by_type.lua", dbData, "int")
			elseif dbId == "plane_by_type" then
				--dumpTable("plane_by_typeX.lua", dbData)
				--dumpTable("plane_by_type.lua", dbData, "int")
			end
		end
			...
			for cId, cData in pairs(uData) do
				if cId == "attribute" then
					for aId, aData in pairs(cData) do
						if aData == "Planes" then
							typeAcf = "Planes"
						elseif aData == "Helicopters" then
							typeAcf = "Helicopters"
						end
					end
				end
