...
GOAP.townControlRadius              = 3000              -- m from town center in which the update function will check presence of more than 1 coalition to define owner
...
function GOAP.phase0_initTerrains()
    ...
    for tId, tData in pairs(GOAP.TerrainDb["towns"]) do
        env.error(("GOAP, phase0_initTerrains: Checking Proxy for: " .. tostring(tData.display_name) .. "\n\n"))
        local pr = GOAP.findProxy(tData, tCopy)
        if pr then
            tData.proxy = pr
        else
            tId = nil -- removing territory from the net
        end
    end
