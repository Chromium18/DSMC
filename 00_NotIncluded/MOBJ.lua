...
-- ## LOCAL VARIABLES
MOBJloaded						= false
local BugFixed					= false -- TILL ED DOESN'T SOLVE THE SCENERY DESTRUCTION BUG, MP SIDE
...
function updateMapObject(missionEnv, tblDeadScenObj)	
	if table.getn(tblDeadScenObj) > 0 then
		...
		for tgId, tgData in pairs (missionEnv.trigrules) do
			if tgData.comment == "SAVE_Scenery_Persistence" then
				currentTrigNum = tgId
		...
		if not currentTrigNum then 
			...
			missionEnv.trigrules[currentTrigNum] = {
				["rules"] = {},
				["eventlist"] = "",
				["comment"] = "SAVE_Scenery_Persistence",
				["actions"] = {},			
		...
		missionEnv.trig.actions[currentTrigNum] = ""		
		...
		for ds_id, ds_data in pairs(tblDeadScenObj) do			
			if ds_data.objId then
				...
				for zoneNum, zoneData in pairs(missionEnv.triggers.zones) do
					if zoneData.zoneId == currentZoneId then
				...
				if okDoDestZone then
					if not missionEnv.triggers.zones[currentZoneNum] then
						missionEnv.triggers.zones[currentZoneNum] = {
							...
							["hidden"] = true,
							["name"] = "DSMC_ScenDest_" .. currentZoneId,
						}
						
						--create trig.actions addon
						if BugFixed == true then
							missionEnv.trig.actions[currentTrigNum] = missionEnv.trig.actions[currentTrigNum] ..  "a_scenery_destruction_zone(" .. currentZoneNum .. ", 100);"
						else
							missionEnv.trig.actions[currentTrigNum] = missionEnv.trig.actions[currentTrigNum] ..  "a_explosion(" .. currentZoneNum .. ", 1, " .. tostring(ExplosionPower) .. ");"
						end

						--populate trigger rules
						if BugFixed == true then
							missionEnv.trigrules[currentTrigNum].actions[actionsId] = {
								["meters"] = 1000,
