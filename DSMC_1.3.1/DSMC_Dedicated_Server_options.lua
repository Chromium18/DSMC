...
DSMC_AutosaveProcess_min		= 1			-- minutes, number, from 2 to 480. DSMC_AutosaveProcess must be true.
...
-- * if you set the number value, DSMC will save the scenery & restart the newly saved mission every "n" hours if no clients are connected (without closing the server). If clients are connected, it will retry every 10 min until all the clients are gone (a message will be propted to anyone online every 10 mins calling for RTB and disconnect).
