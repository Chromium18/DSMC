...
local function loadDSMCHooks()
	...
	UTIL.createWeaponsDb()

	if UTIL and SAVE then

		DSMCloader = true
		local autosaveVal = nil
		if DCS.getConfigValue("norender") == 0 or forceServerMode == true then
			--writeDebugBase(DSMC_ModuleName .. ": autosaveVal, server mode")

			local dso_fcn, dso_err = dofile(DSOdir .. "DSMC_Dedicated_Server_options.lua")
			--writeDebugBase(DSMC_ModuleName .. ": autosaveVal, file loaded")
			if DSMC_AutosaveProcess == true then
				writeDebugBase(DSMC_ModuleName .. ": autosaveVal, DSMC_AutosaveProcess is true")
				autosaveVal = true
			end
		else
			local opt_fcn, opt_err = dofile(lfs.writedir() .. "Config/options.lua")
			--writeDebugBase(DSMC_ModuleName .. ": autosaveVal, options loaded")
			if options then
				for opt_id, opt_data in pairs(options) do
					if opt_id == "plugins" then
						for pl_id, pl_data in pairs (opt_data) do
							if pl_id == "DSMC" then				
								--writeDebugBase(DSMC_ModuleName .. ": autosaveVal, options DSMC identified: " .. tostring(pl_data.ATRL))		
								autosaveVal = pl_data.ATRL
							end
						end
					end
				end
...
local function recoverAutosave()
	if 	UTIL.fileExist(DSMCfiles .. "tblDeadUnits.lua") and 
		...
		UTIL.fileExist(DSMCfiles .. "tblWarehousesContent.lua") and
		UTIL.fileExist(DSMCfiles .. "tblConquer.lua") and
		UTIL.fileExist(DSMCfiles .. "tblCoaChanges.lua") then
...
local function extractMissionNameDSMC(path)
    -- Estrae il nome del file che inizia con DSMC
    local filename = string.match(path, "([\\/]DSMC[^\\/]+)$")
    if filename then
        -- Rimuove lo slash iniziale
        filename = filename:sub(2)
        -- Rimuove l'estensione, se presente (.miz, .sav o qualsiasi altra)
...
local function startDSMCprocess()
	...
	if UTIL and SAVE then
		...
		writeDebugBase(DSMC_ModuleName .. ": opt_EMDB_var: " ..tostring(opt_EMDB_var))
		...
		if UTIL.fileExist(DSMCdir .. "CRST" .. ".lua") == true and CRST_var == true then
			CRST 						= require("CRST")
			writeDebugBase(DSMC_ModuleName .. ": loaded in CRST module")
		end
		if UTIL.fileExist(DSMCdir .. "WRHS" .. ".lua") == true and WRHS_var == true then
			WRHS 						= require("WRHS")
			writeDebugBase(DSMC_ModuleName .. ": loaded in WRHS module")
		end
		if UTIL.fileExist(DSMCdir .. "WTHR" .. ".lua") == true and WTHR_var == true then
			WTHR 						= require("WTHR")
			writeDebugBase(DSMC_ModuleName .. ": loaded WTHR module")
		end			
		if UTIL.fileExist(DSMCdir .. "TMUP" .. ".lua") == true and TMUP_var == true then
			TMUP 						= require("TMUP")
			writeDebugBase(DSMC_ModuleName .. ": loaded in TMUP module")
		end
		if UTIL.fileExist(DSMCdir .. "SPWN" .. ".lua") == true and SPWN_var == true then
			SPWN 						= require("SPWN")
			...
			if string.sub(loadedMizFileName,1,4) == StartFilterCode then
				...
				if SAVE.tempEnv.mission and SAVE.tempEnv.warehouses and SAVE.tempEnv.dictionary and SAVE.tempEnv.mapResource then
					...
					if baseUcounter and baseGcounter then
						...
						writeDebugDetail(DSMC_ModuleName .. ": DSMC_mizName = " .. tostring(loadedMizFileName))
						UTIL.inJectCode("DSMC_code", "DSMC_code = " .. tostring(filtered) )
						UTIL.inJectCode("DSMC_mizName", "DSMC_mizName = " .. '"' .. tostring(loadedMizFileName) .. '"' )
							...
							if found == false then
								...
								for wSec, wData in pairs(wTbl) do
									if wData.unlimitedMunitions == false and wData.unlimitedAircrafts == false then
										...
										-- acf
										for acCategory, acNames in pairs(wData.aircrafts) do
											for acName, acData in pairs(acNames) do
												
												local data = acData.wsType
												for attID, attData in pairs(data) do
													if attID > 4 then
														data[attID] = nil
													end
												end
	
												wsTypesTbl[#wsTypesTbl+1] = data
											end
										end
	
	
										-- acf
										for xId, xData in pairs(wData.weapons) do
	
											local data = xData.wsType
											for attID, attData in pairs(data) do
												if attID > 4 then
													data[attID] = nil
												end
						...
						UTIL.inJectTable("wsTypesTbl", wsTypesTbl)
...
local function loadtables()
	...
	if debugProcessDetail == true then
		...
		UTIL.dumpTable("tblDeadUnits.lua", tblDeadUnits, "int")
		UTIL.dumpTable("tblDeadScenObj.lua", tblDeadScenObj, "int")
		UTIL.dumpTable("tblUnitsUpdate.lua", tblUnitsUpdate, "int")
		UTIL.dumpTable("tblAirbases.lua", tblAirbases, "int")
		UTIL.dumpTable("tblSpawned.lua", tblSpawned, "int")
		UTIL.dumpTable("tblConquer.lua", tblConquer, "int")
		UTIL.dumpTable("tblCoaChanges.lua", tblCoaChanges, "int")		
		UTIL.dumpTable("tblWarehouseChangeCoa.lua", tblWarehouseChangeCoa, "int")
		UTIL.dumpTable("tblWarehousesContent.lua", tblWarehousesContent, "int")
	end
end

local function makefirstmission(missionPath)
...
local function batchSaveProcess()	
	...
	if SAVE and UTIL then
		...
		if tblAirbases and tblDeadUnits and tblDeadScenObj and tblUnitsUpdate and tblWarehousesContent and tblSpawned and tblConquer then
...
local function saveOnDisconnect()
...
function DSMC.onMissionLoadBegin()
	...
	local lpath = DCS.getMissionFilename()
	if lpath and string.find(lpath, StartFilterCode) then
		loadedMissionPath = lpath
		if string.find(lpath, StartFilterCode) then
			loadedMissionPath = lpath
			writeDebugBase(DSMC_ModuleName .. ": onMissionLoadBegin, loadedMissionPath: " .. tostring(loadedMissionPath))	
			if string.sub(DCS.getMissionName(),1,4) == "temp" then
				loadedMizFileName = extractMissionNameDSMC(loadedMissionPath)
	...
	writeDebugBase(DSMC_ModuleName .. ": onMissionLoadBegin, done")	
end

function DSMC.onSimulationStart()
	if string.find(DCS.getMissionFilename(), StartFilterCode) then
		startDSMCprocess()
	end
end
			...
			if string.sub(message, 1, 3) == "tbl" then	-- this will import & save any table that starts with "tbl"					
				...
				if not str_err then -- check errors  -- == true 
					...
					elseif tableName == "tblUnitsUpdate" then
						...
						writeDebugDetail(DSMC_ModuleName .. ": recognized & saved " .. tostring(tableName))	
					elseif tableName == "tblWarehousesContent" then
						UTIL.saveTable(tableName, tblWarehousesContent, DSMCfiles)
						writeDebugDetail(DSMC_ModuleName .. ": recognized & saved " .. tostring(tableName))							
...
loadDSMCHooks()
writeDebugDetail(DSMC_ModuleName .. ": HOOKs code loaded")
