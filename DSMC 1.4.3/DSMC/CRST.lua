...
function doStatics(missionEnv, tblDeaths)
	...
	for id, deadData in pairs (tblDeaths) do
		if deadData.coalitionID ~= 0 and deadData.objCategory ~= 3 and deadData.objCategory ~= 6 and deadData.unitShip ~= true and deadData.unitInfantry ~= true and deadData.staticTable and deadData.staticTable ~= "none" then -- not cargos
			...
			then
				...
				if correctCoalition then
					if missionEnv.coalition[correctCoalition] and missionEnv.coalition[correctCoalition]["country"] then
