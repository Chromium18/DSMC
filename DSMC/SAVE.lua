...
function updateAirbaseTable(missionEnv)
	for admId, admData in pairs(tblAirbases) do
		...
		elseif admData.desc.category == 2 then
			...
			if unitDef then
				if admData.desc.attributes.buildings == true then
				...
				HOOK.writeDebugDetail(ModuleName .. " updateAirbaseTable: type not available")
			end
		end
...
function IncludeSpawned(missionEnv, tbl, dictEnv)
	if SPWN and missionEnv and tbl and dictEnv then
		local lthStr, lthStrErr = SPWN.doSpawned(missionEnv, tbl, dictEnv)		
	...
	for coalitionID,coalition in pairs(missionEnv["coalition"]) do
		for countryID,country in pairs(coalition["country"]) do
			for attrID,attr in pairs(country) do
				if (type(attr)=="table") then
					if attrID == "plane" then
						HOOK.writeDebugDetail(ModuleName .. ": plane found, skip")
...
function updateWarehouse(tblLogistic, tbl)
	if WRHS and tblLogistic and tbl and WRHS.WRHSloaded == true then		
		...
		if not lthStrErr then
			HOOK.writeDebugDetail(ModuleName .. ": updateWarehouse, elabLogistic errors: " .. tostring(lthStr))
		end
...
function save() 
	...
	if mixFun and wrhsFun and dictFun and mResFun then
		...
		if HOOK.SPWN_var == true then
			IncludeSpawned(env.mission, tblSpawned, dict_env.dictionary)
		...
		if HOOK.WRHS_var == true then
			...
			for sId, sValue in pairs(dict_env.dictionary) do 
				if sId == sortieIndex then
					sortieValue = sValue
					dict_env.dictionary[sId] = ""
				end
			end
			
			if sortieValue == "warehouse reset" and HOOK.SBEO_var == true then
				UTIL.getZeroedAirbase(wrhs_env.warehouses)

			elseif sortieValue == "warehouse auto" and HOOK.SBEO_var == true then
				--UTIL.getZeroedAirbase(wrhs_env.warehouses)
...
function getMizFiles(loadedMissionPath)
	...
	HOOK.writeDebugDetail(ModuleName .. ": minizip.unzOpen opening=" .. tostring(loadedMissionPath))
