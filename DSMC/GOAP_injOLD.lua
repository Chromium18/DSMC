        ...
        if tblTerrain then
            if tblTerrain.towns then
                ...
                if phase_index > #tblTerrain.towns then
                    ...
                    GOAP.changePhase()
                    timer.scheduleFunction(GOAP.performPhaseCycle, {}, timer.getTime() + 1)
                    ...
                    timer.scheduleFunction(GOAP.performPhaseCycle, {}, timer.getTime() + 0.1)
                        ...
                        for groupID,group in pairs(attr["group"]) do
                            if (group) then
                                ...
                                if gName then
                                    local gTbl = GOAP.groupTableCheck(gName)
                                    if gTbl then
                                        env.info(("GOAP, phaseB_createIntelBase: adding static " .. tostring(gName)))   
                                        
                                        local coa = gTbl:getCoalition()
                                        local unit = gTbl:getUnit(1)
                                        if unit then
                                            local t_Id = unit:getID()
                                            local tgtType = unit:getTypeName()
                                            local tgtPos = unit:getPosition().p
                                            local tgtLife = unit:getLife0()                                 
            ...
            if phase_index > #currentGroupTable then
                ...
                for coa, coaData in pairs(GOAP.intel) do
                    for gId, gData in pairs(coaData) do
                        if not gData.static then -- never remove statics
...
GOAP.tblActionPlan = {
    ...
    ["Scout_Territory"] = {
        ["preq"] = {"RECON_avail", "Negative_cost"},
        ["gain"] = "Information",
        ["action"] = "Group.goToTown", 
        ["subaction"] = "Group.haltOnContact", 
        ["basecost"] = 2,
...
function GOAP.groupTableCheck(group) -- STILL NEEDED
    if group then
    ...
    if tData then
        if tData.object then
            if type(tData.object) == "table" and tData.object:getCategory() == 1 then
                ...
                env.info(("GOAP.addTgtToKnownTarget, missing object info, probable static"))
                return false                
            end
        else
