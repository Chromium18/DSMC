...
-- ## LOCAL VARIABLES
SPWNloaded						= false
KeepGroupName					= false -- if true a spawned group will retain it's name in the saved files... but it means that CTLD can overwrite it! if overwritten (by spawning another group) the original one will instantly disappear from the mission

-- ## MANUAL TABLES


-- ## ELAB FUNCTION
function doSpawned(missionEnv, tblSpawned, dictEnv)
					...
					if groupTable and correctCoalition and correctCountry then
						...
						for uId, uData in pairs(sgData.gUnits) do
							if uData.uAlive == true then
								...
								tblDictEntries[UnitDictEntry] = tostring(uData.uType) .. "_" .. tostring(MaxDict)
									...
									sgUnits[#sgUnits + 1] =
													{
														...
														["shape_name"] = uData["uDesc"]["typeName"],
														["unitId"] = uData.uID,
														["y"] = uData.uPos.z,
														["x"] = uData.uPos.x,
														["name"] = UnitDictEntry,
