...
local addedunits				= 0
local tblSlots					= {}
local MaxSlotsPerHeliport		= 8 -- maximum number of created slots per heliport that could be generated. Not related to airport/airbase
			...
			for coalitionID,coalition in pairs(mission["coalition"]) do
				if string.lower(sData.coaID) == string.lower(coalitionID) then
					...
					for countryID,country in pairs(coalition["country"]) do
						if sData.cntyID == countryID then			
											...
											for i=1, sData.numUnits do
												for acfId, acfData in pairs(standardUnitTypes) do
													if acfId == sData.acfType then
														...
														HOOK.writeDebugDetail(ModuleName .. ": createGroups adding unit, callsign.name: " .. tostring(mtext .. tostring(addedGroups) .. tostring(i)))
							...
							if (group) then						
								for unitID, unit in pairs(group["units"]) do
									if unit.skill == "Client" or unit.skill == "Player" then
										...
										HOOK.writeDebugDetail(ModuleName .. ": addSlot killed group")	
									end
								end
							end
						end
												...
												if (group) then
													for unitID,unit in pairs(group["units"]) do
														if tonumber(unit.unitId) == tonumber(afbId) then
															HOOK.writeDebugDetail(ModuleName .. ": addSlot, found heliport: " .. tostring(unit.unitId))
															...
															if heading_val and x_val and y_val and link_val and coa and link_type_val then
																if afbData.unlimitedAircrafts then
																	...
																	tblSlots[#tblSlots+1] = {h= heading_val, x=x_val, y = y_val, link = link_val, linkType = link_type_val, cntyID = countryID, coaID = coa, acfType = "Ka-50", numUnits = 2}
																	tblSlots[#tblSlots+1] = {h= heading_val, x=x_val, y = y_val, link = link_val, linkType = link_type_val, cntyID = countryID, coaID = coa, acfType = "Mi-8MT", numUnits = 2}
																	tblSlots[#tblSlots+1] = {h= heading_val, x=x_val, y = y_val, link = link_val, linkType = link_type_val, cntyID = countryID, coaID = coa, acfType = "UH-1H", numUnits = 2}
																	tblSlots[#tblSlots+1] = {h= heading_val, x=x_val, y = y_val, link = link_val, linkType = link_type_val, cntyID = countryID, coaID = coa, acfType = "SA342L", numUnits = 2}
																			...
																			for acfId, acfData in pairs(catData) do
																				for aId, aData in pairs(standardUnitTypes) do
																					...
																					if acfId == aId then
																						...
																						if numberFlights > 0 and numberFlights < 1 then
																							tblSlots[#tblSlots+1] = {h= heading_val, x=x_val, y = y_val, link = link_val, linkType = link_type_val, cntyID = countryID, coaID = coa, acfType = acfId, numUnits = 1}
																							HOOK.writeDebugDetail(ModuleName .. ": addSlot, creating 1 group, single ship")
																						elseif numberFlights == 1 then
																							tblSlots[#tblSlots+1] = {h= heading_val, x=x_val, y = y_val, link = link_val, linkType = link_type_val, cntyID = countryID, coaID = coa, acfType = acfId, numUnits = 2}
																							HOOK.writeDebugDetail(ModuleName .. ": addSlot, creating 1 group, 2ship")																							
																						elseif numberFlights < 2 and numberFlights > 1 then
																							tblSlots[#tblSlots+1] = {h= heading_val, x=x_val, y = y_val, link = link_val, linkType = link_type_val, cntyID = countryID, coaID = coa, acfType = acfId, numUnits = 3}
																							HOOK.writeDebugDetail(ModuleName .. ": addSlot, creating 1 group, 3ship")
																						elseif numberFlights >= 2 then
