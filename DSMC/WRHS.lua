...
WRHSloaded						= false
tblWarehouses					= nil
local minWpnSupply				= 30	-- min amount to skip weapon supply
local supplyAmount				= 10    -- amount of wpn (per type) supplied by the production site
...
-- ## ELAB FUNCTION
function consolidateLogistic(curr_tblLogistic)
	local temp_tblLogistic ={}
	HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic started")
	for oId, oData in pairs(curr_tblLogistic) do
		HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic checking: " .. tostring(oId))
		...
		if table.getn(temp_tblLogistic) > 0 then
			for cId, cData in pairs(temp_tblLogistic) do
				if oData.placeId == cData.placeId then
					found = true
					tempTbl = cData
					HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, found existing temp_tblLogistic entry for  " .. tostring(cData.placeId))
				end
			end
		end

		if found == false then
			tempTbl = {placeId = oData.placeId, jet_fuel = {InitFuel = 0}, weapons = {}, aircrafts = {}}
			HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, no previous temp_tblLogistic entry for  " .. tostring(oData.placeId))
		end
		-- fuel ops
		if oData.action == "departure" then
			tempTbl.jet_fuel.InitFuel = tempTbl.jet_fuel.InitFuel - (math.floor(oData.fuel/100)/10)
			HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, fuel recorded, departure")
		elseif oData.action == "arrival" then
			tempTbl.jet_fuel.InitFuel = tempTbl.jet_fuel.InitFuel + math.floor(oData.fuel/100)/10
			HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, fuel recorded, arrival")
		end
		...
		if oData.action == "departure" then
			...
			for dId, dData in pairs(oData.ammo) do
				HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, wpn recorded, departure: " .. tostring(dData.wsString))
				...
				for rId, rData in pairs(wpnTemp) do
					if dData.wsString == rData.wsString then
						...
						wfound = true
						HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, found wpn in wpnTemp, removing from there")
					end
				end

				if wfound == false then
					HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, wpn is new in wpnTemp, removing from there")
		...
		elseif oData.action == "arrival" then
			local wpnTemp = tempTbl.weapons
			for dId, dData in pairs(oData.ammo) do
				HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, wpn recorded, arrival: " .. tostring(dData.wsString))
				...
				for rId, rData in pairs(wpnTemp) do
					if dData.wsString == rData.wsString then
						rData.amount = rData.amount + dData.amount
						wfound = true
						HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, found wpn in wpnTemp, adding there")
					end
				end

				if wfound == false then
					HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, wpn is new in wpnTemp, adding there")
					wpnTemp[#wpnTemp+1] = {amount = dData.amount, wsString = dData.wsString}
				end
		...
		if oData.action == "departure" then
			...
			for aId, aNumber in pairs(tmpAcf) do
				if aId == oData.acf then
					afound = true
					HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, found acf in tmpAcf, remove action, curVal = " ..tostring(aNumber))
					aNumber = aNumber - 1
					HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, found acf in tmpAcf, remove action, newVal = " ..tostring(aNumber))
			...
			if afound == false then
				tmpAcf[oData.acf] = -1
				HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, acf is new in tmpAcf, removing there")
			end
		...
		elseif oData.action == "arrival" then
			...
			for aId, aNumber in pairs(tmpAcf) do
				if aId == oData.acf then
					afound = true
					HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, found acf in tmpAcf, add action, curVal = " ..tostring(aNumber))
					aNumber = aNumber + 1
					HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, found acf in tmpAcf, add action, curVal = " ..tostring(aNumber))
					tmpAcf[oData.acf] = aNumber
				end
			...
			if afound == false then
				tmpAcf[oData.acf] = 1
				HOOK.writeDebugDetail(ModuleName .. ": consolidateLogistic, acf is new in tmpAcf, adding there")
			end
	...
	UTIL.dumpTable("WRHS.consolidateLogistic.lua", temp_tblLogistic)
	return temp_tblLogistic

end


function elabLogistic(inj_tblLogistic, inj_tempWarehouses)
	...
	if tblLogistic and tempWarehouses then
			...
			for afbType, afbIds in pairs(tempWarehouses) do
				for afbId, afbData in pairs(afbIds) do
					...
					if tonumber(afbId) == tonumber(LogData.placeId) then						
						HOOK.writeDebugDetail(ModuleName .. ": elabLogistic placeId ok: " .. tostring(afbId))
						...
						if afbData.unlimitedFuel == false then
							if LogData.jet_fuel.InitFuel and type(LogData.jet_fuel.InitFuel) == "number" then
								if LogData.jet_fuel.InitFuel < 0 then
									HOOK.writeDebugDetail(ModuleName .. ": elabLogistic to remove fuel: " .. tostring(LogData.jet_fuel.InitFuel))
									...
									if qty then
										...
										HOOK.writeDebugDetail(ModuleName .. ": elabLogistic ERROR, so fuel removed: " ..tostring(LogData.jet_fuel.InitFuel) .. "\n")
									end

								else
									afbData.jet_fuel.InitFuel = afbData.jet_fuel.InitFuel + LogData.jet_fuel.InitFuel 
									HOOK.writeDebugDetail(ModuleName .. ": elabLogistic fuel added: " ..tostring(LogData.jet_fuel.InitFuel) .. "\n")
								end
							end
						end
						...
						if LogData.aircrafts then
								...
								for acCategory, acNames in pairs(afbData.aircrafts) do
									for acName, acData in pairs(acNames) do
										if acName == aName then
											...
											HOOK.writeDebugDetail(ModuleName .. ": elabLogistic adjusted acf type: " .. tostring(aName) .. ", qty: " .. tostring(aNumber) .. "\n")	
										end
									end
								end
							...
							HOOK.writeDebugDetail(ModuleName .. ": elabLogistic no aircraft variable")	
						end
						
						--ammo  
						if afbData.unlimitedMunitions == false then
							if LogData.weapons then
								if table.getn(LogData.weapons) > 0 then
									for rId, rData in pairs(LogData.weapons) do
										HOOK.writeDebugDetail(ModuleName .. ": elabLogistic wpn request: " .. tostring(rData.wsString)  .. ", qty: " .. tostring(rData.amount))	
											...
											if w_wsType_str then
												if w_wsType_str == rData.wsString then
													...
													if rData.amount < 0 then
														HOOK.writeDebugDetail(ModuleName .. ": elabLogistic to remove ammo: " .. tostring(rData.amount))
														
														-- try to catch supplier!
														HOOK.writeDebugDetail(ModuleName .. ": current ammo amount: " .. tostring(wData.initialAmount))
														local qty = executeAmmoSupply(code_id, afbData.suppliers, -rData.amount, rData.wsString, afbData.coalition)
														if qty then
															HOOK.writeDebugDetail(ModuleName .. ": recovered by supply: " .. tostring(qty))	
															wData.initialAmount = wData.initialAmount + rData.amount + qty
															HOOK.writeDebugDetail(ModuleName .. ": new fuel amount: " .. tostring(wData.initialAmount))	
					
														else
															wData.initialAmount = wData.initialAmount + rData.amount
															HOOK.writeDebugDetail(ModuleName .. ": elabLogistic ERROR, so ammo removed: " ..tostring(rData.amount) .. "\n")
														end
					
													else
														wData.initialAmount = wData.initialAmount + rData.amount
														HOOK.writeDebugDetail(ModuleName .. ": elabLogistic ammo added: " ..tostring(rData.amount) .. "\n")
													end
		...
		HOOK.writeDebugDetail(ModuleName .. ": elabLogistic cycle done")
...
function executeAmmoSupply(code_ori, tbl_su, req_quantity, req_wsType, coa)
									...
									for wId, wData in pairs(afbData.weapons) do
										...
										if w_wsType_str then
											if w_wsType_str == req_wsType then
												...
												HOOK.writeDebugDetail(ModuleName .. ": executeAmmoSupply evaluate supplier: " .. tostring(afbId) .. ", weapon: " .. tostring(req_wsType) .. ", ammo_avail: " .. tostring(ammo_avail) .. ", ammo_req: " .. tostring(ammo_req) .. ", ammo_rem: " .. tostring(ammo_rem))
						...
						for r_wId, r_wData in pairs(r_afbData.weapons) do
							...
							if w_wsType_str then
								if w_wsType_str == req_wsType then			
									...
									local re_qty = executeAmmoSupply(r_code_cur, r_afbData.suppliers, re_req, req_wsType, coa)
...
function warehouseUpdateCycle(inj_tblLogistic, inj_tempWarehouses)
	HOOK.writeDebugDetail(ModuleName .. ": warehouseUpdateCycle start!")
	local tblTest = consolidateLogistic(inj_tblLogistic)
	local updatedWh = elabLogistic(tblTest, inj_tempWarehouses)
