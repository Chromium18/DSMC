...
function save() 
	...
	if mixFun and wrhsFun and dictFun and mResFun then
		...
		if HOOK.WRHS_var == true then
			...
			if string.find(string.lower(HOOK.loadedMizFileName), "resetwh") then
			...
			elseif string.find(string.lower(HOOK.loadedMizFileName), "create") then
				...
				UTIL.whAutoPopulateDepotsAndProduction(wrhs_env.warehouses, env.mission)
				doPlans = false
			elseif string.find(string.lower(HOOK.loadedMizFileName), "resetNoAcf") then
				HOOK.writeDebugDetail(ModuleName .. ": mission is reset warehouse mode, setting all wh to void")
				UTIL.whAutoZero(wrhs_env.warehouses, false)
				doPlans = false
			elseif string.find(string.lower(HOOK.loadedMizFileName), "reset") then
				HOOK.writeDebugDetail(ModuleName .. ": mission is reset warehouse mode, setting all wh to void")
...
function getNewMissionName(currentName)
	if currentName then
		...
		if type(val) == "number" then
			...
			return strSub
		elseif string.lower(strSub) == "tWh" then
			HOOK.writeDebugDetail(ModuleName .. ": getNewMissionName - recognized warehouse zero and reset miz")
			HOOK.writeDebugDetail(ModuleName .. ": getNewMissionName : " .. currentName .. "_wh_reset.miz")
			return currentName .. "_wh_zeroized.miz"
		elseif string.lower(strSub) == "set" then
			HOOK.writeDebugDetail(ModuleName .. ": getNewMissionName - recognized warehouse reset miz")
			HOOK.writeDebugDetail(ModuleName .. ": getNewMissionName : " .. currentName .. "_wh_zeroized.miz")
			return currentName .. "_wh_zeroized.miz"
		elseif string.lower(strSub) == "ate" then
			HOOK.writeDebugDetail(ModuleName .. ": getNewMissionName - recognized warehouse create miz")
			HOOK.writeDebugDetail(ModuleName .. ": getNewMissionName : " .. currentName .. "_wh_created.miz")
			return currentName .. "_wh_created.miz"
		else
