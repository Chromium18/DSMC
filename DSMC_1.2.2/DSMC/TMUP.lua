...
function updateStTime(missionEnv)
	...
	HOOK.writeDebugDetail(ModuleName .. ": updateStTime 1")
	...
	if CURstartDateYear then
		
		--local deltaTime = nil
		local added = false
		if tonumber(CURstartDateYear) < 1970 then
			CURstartDateYear = CURstartDateYear + 100 -- one century added
			added = true
		end

		HOOK.writeDebugDetail(ModuleName .. ": updateStTime 2: CURstartDateDay=" .. tostring(CURstartDateDay) .. ", CURstartDateMonth=" .. tostring(CURstartDateMonth) .. ", CURstartDateYear=" .. tostring(CURstartDateYear) .. ", CURstartTime=" .. tostring(CURstartTime) )
		...
		HOOK.writeDebugDetail(ModuleName .. ": failed to identify year")
	end
end
